...
def get_data():
    file = glob.glob(".\\分段数据\\*.log")
    file = open(file[0], 'r', encoding="utf-8")
    k = 0
    list1 = []
    time_list=[]
    list = file.readlines()
    ...
    for j in range(k):
        o_fd = open('./分段数据/%s.txt' % (time_list[j]), 'w', encoding="utf-8")
...
def handle_data():
    file = glob.glob(".\\分段数据\\YF_GPS_*.txt")
    ...
    for i in range(len(file)):
        ...
        for k in range(len(list)):
            if "YF_GPS" not in list[k] and "$" not in list[k]:
                list1.append(list[k])
        path = file[i].split(".")[1].split("\\")[2]

        file2 = open(os.path.join(os.getcwd(), ".\\分段数据\\原始数据_%s.txt" % path), 'w', encoding="utf-8")
        file2.writelines(list1)

def rm_file():
    for file in glob.glob(os.path.join(r'分段数据', '*.txt')):
        os.remove(file)
